...
pins. The ESP8266 is connected to the internet through a Wi-Fi router based on the IEEE 802.11 b/g/n standards because the ESP8266 only supports the 2.4 Ghz, which is secured 
by WPA/WPA2 encryption. The ThingSpeak dashboard through HTTPS is secured by the cloud server TLS, the data from sensors is transferred from the ESP8266 side to the ThingSpeak
cloud server through HTTP/TCPIP port 80 authenticated by the API key.
...
•	Arduino microcontroller continuously reads:
    o	Digital input from the PIR sensor, Analog input from the LM35 sensor.

## Step 2: Local Processing and Control
•	The Arduino processes the sensor data:
    o	Motion status is determined (0 = no motion, 1 = motion detected).
    o	LM35 voltage output is used to measure the temperature.
•	Based on predefined thresholds:
    o	Relay module is activated to turn the fan ON or OFF
    o	LED indicators are switched to reflect motion detection
